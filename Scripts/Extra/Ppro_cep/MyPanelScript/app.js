...
(() => {
    ...
    function pickPathPS(opts, cb) {
        if (!hasNode || !cp || process.platform !== 'win32') {
            cb(null);
            return;
        }
        opts = opts || {};
        ...
        var ps = parts.filter(function (x) {
            return x && x.length;
        }).join("; ");
    ...
    function onPproTimingsReady(evt) {
        ...
        try {
            ...
            if (Array.isArray(outPaths) && outPaths.length) {
                ...
                // NEW: мгновенная смена фокуса на панель задач и обратно перед импортом для обновления кэша premiere (focusbounce.exe)
                try {
                    ...
                    const FOCUS_EXE = extRoot + '/lib/focusbounce.exe';
                    
                    const runImport = function () {
                        const jsxCall = "try{typeof importSrtFromList==='function'?importSrtFromList():'importSrtFromList not found'}catch(e){'ERROR: '+e}";
                        cs.evalScript(jsxCall, function (ret) {
                            if (ret && String(ret).length) logJSX('[importSrtFromList] ' + String(ret));
                        });
                    };
                    
                    if (fs.existsSync(FOCUS_EXE)) {
                        cp.execFile(FOCUS_EXE, [], {
                            windowsHide: true
                        }, function (err, stdout, stderr) {
                            if (stdout && String(stdout).trim()) logJS('[focusbounce] ' + String(stdout).trim());
                            if (stderr && String(stderr).trim()) logJS('[focusbounce:stderr] ' + String(stderr).trim());
                            if (err) logJS('⚠️ focusbounce error: ' + (err.message || err));
                            runImport();
                        });
                    }
                    else {
                        logJS('⚠️ focusbounce.exe не найден по пути: ' + FOCUS_EXE + ' — продолжаю без него');
                        runImport();
                    }
                }
                catch (ee) {
                    logJS('⚠️ Не удалось выполнить focusbounce: ' + (ee && ee.message ? ee.message : ee));
                    // даже при исключении — всё равно запускаем импорт
                    try {
                        const jsxCall = "try{typeof importSrtFromList==='function'?importSrtFromList():'importSrtFromList not found'}catch(e){'ERROR: '+e}";
                        cs.evalScript(jsxCall, function (ret) {
                            if (ret && String(ret).length) logJSX('[importSrtFromList] ' + String(ret));
    ...
    function pickFileDialog(title, extsArr) {
        if (!window.cep || !cep.fs || typeof cep.fs.showOpenDialog !== 'function') {
            alert('cep.fs недоступен');
            return null;
        }
    ...
    const srtBuildingModule = (function () {
        ...
        function writeFileSafe(absPath, content) {
            try {
                ...
                if (console && console.warn) console.warn(`Не удалось записать файл "${absPath}": ${e.message}`);
                return false;
            }
        }
        ...
        function buildSrtFromJsonAndCsv(jsonPath, csvPath) {
            const {
                starts,
                ends
            } = readAndValidateTimings(jsonPath);
         ...
         */
        function buildSingleSrtFromJsonAndCsvInsideTempFolder(jsonPath, csvPath, targetCsvTitle) {
        ...
        return {
            ...
            buildSingleSrtFromJsonAndCsvInsideTempFolder
        };
    })();
